...
from solvers.symbols import t, s, t0
...
def inverseLaplaceProcess(item):
    ...
    for a in sp.preorder_traversal(expr_t):
        if isinstance(a, sp.Heaviside):
            if a.args[0] == t:
                expr_t = expr_t.subs({sp.Heaviside(a.args[0]) : 1})
            else:
                expr_t = expr_t.subs({sp.Heaviside(a.args[0]) : sp.Heaviside(a.args[0], 1)})
...
def solveLaplace2(compiled, depth = 1, simulatorState = None, debugLog=0):
    ...
    try:
        ...
        for _ in range(depth):
            ...
            for states, equations, conditions, edge_cases in permutations:
                ...
                for conds in conditions.values():
                    for cond in conds:
                        importantVariables.update(cond.atoms(sp.Symbol).intersection(variables))
                ...
                try:
                    ...
                    for sol in sols:
                        ...
                        for var, expr in deepcopy(sol).items():
                            atoms = expr.atoms(Symbol).intersection(variables)
                            if atoms:
                                for sym in atoms:
                        ...
                        sol = {var : sp.simplify(expr.subs(independent_sol)) for var, expr in sol.items()}
                        ...
                        state_change = {}
                        for state, conds in edge_cases.items():
                            
                            changes = []
                            for cond in conds:
                                cond_ = cond.subs(sol_t)
                                print(cond_)
                                sols_ = sp.solve(cond_, t, list=True)
                                print(sols_)
                                changes.extend([sp.simplify(sol_) for sol_ in sols_])    
                                                            
                            state_change.update({state : changes})
